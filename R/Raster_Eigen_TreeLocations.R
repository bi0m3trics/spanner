...
#' @return sf object An `sf` object containing the following seed information: `TreeID`,
...
#'
#' @export
get_raster_eigen_treelocs <- function(las, res = 0.05, pt_spacing = 0.0254, dens_threshold = 0.2,
                                      ...
                                      max_dia = 0.5, SDvert = 0.25, n_best = 25, n_pts = 20,
                                      inliers = 0.9, conf = 0.99, max_angle = 20) {
  # Parallel processing setup
  ncpu <- lidR::get_lidr_threads()
  ...
  vert_temp <- spanner::eigen_metrics(slice_extra, radius = neigh_sizes[1], ncpu = ncpu)
  if (nrow(vert_temp) == 0) stop("Problem calculating verticality...!", call. = FALSE)
  slice_extra <- lidR::add_lasattribute(slice_extra, as.numeric(vert_temp$Verticality), name = "verticality", desc = "verticality")
  slice_extra@data$verticality[is.na(slice_extra@data$verticality)] <- 0.5

  # Create canopy cover raster
  ...
  cnt_local <- spanner:::C_count_in_disc(X = slice_extra@data$X, Y = slice_extra@data$Y, x = slice_extra@data$X, y = slice_extra@data$Y, radius = neigh_sizes[2], ncpu = ncpu)
  cnt_large <- spanner:::C_count_in_disc(X = slice_extra@data$X, Y = slice_extra@data$Y, x = slice_extra@data$X, y = slice_extra@data$Y, radius = neigh_sizes[3], ncpu = ncpu)
  slice_extra <- lidR::add_lasattribute(slice_extra, as.numeric(cnt_local / cnt_large), name = "relative_density", desc = "relative density")

  # Gridding relative density
  message("Gridding relative density... (6/14)\n")
  density_grid <- terra::rast(lidR::grid_metrics(slice_extra, ~stats::median(relative_density, na.rm = TRUE), res = res, start = c(min(slice_extra@data$X), min(slice_extra@data$Y))))
  density_polygon <- terra::as.polygons(terra::classify(density_grid, rbind(c(0, dens_threshold, 0), c(dens_threshold, 1, 1))), dissolve = TRUE)
  density_polygon <- st_as_sf(density_polygon)
  ...
  density_polygon <- sf::st_buffer(sf::st_buffer(density_polygon, 1.5), -1.475)
  ...
  # Gridding verticality
  message("Gridding verticality... (7/14)\n")
  verticality_grid <- terra::rast(lidR::grid_metrics(slice_extra, ~stats::quantile(verticality, 0.5, na.rm = TRUE), res = res))
  verticality_polygon <- terra::as.polygons(terra::classify(verticality_grid, rbind(c(0, eigen_threshold, 0), c(eigen_threshold, 1, 1))), dissolve = TRUE)
  verticality_polygon <- st_as_sf(verticality_polygon)
  ...
  verticality_polygon <- sf::st_buffer(sf::st_buffer(verticality_polygon, 1.5), -1.475)
  ...
  # Merging relative density and verticality
  ...
  # Filtering merged polygon
  message("Filtering merged polygon... (9/14)\n")
  verticalitySD_grid <- terra::rast(grid_metrics(slice_extra, ~stats::sd(verticality), res = res))
  merged$SDvert <- round(terra::extract(verticalitySD_grid, terra::vect(sf::as_Spatial(merged)), fun = function(x) { stats::median(x, na.rm = TRUE) })[, 2], 2)
  Z01 <- terra::rast(grid_metrics(slice_extra, res = res, ~stats::quantile(Z, 0.01)))
  merged$lowHGT <- round(terra::extract(Z01, terra::vect(sf::as_Spatial(merged)), fun = function(x) { stats::quantile(x, 0.05, na.rm = TRUE) })[, 2], 2)
  Z99 <- terra::rast(grid_metrics(slice_extra, res = res, ~stats::quantile(Z, 0.99)))
  merged$highHGT <- round(terra::extract(Z99, terra::vect(sf::as_Spatial(merged)), fun = function(x) { stats::quantile(x, 0.99, na.rm = TRUE) })[, 2], 2)
  Z50 <- terra::rast(grid_metrics(slice_extra, res = res, ~stats::quantile(Z, 0.5)))
  merged$medHGT <- round(terra::extract(Z50, terra::vect(sf::as_Spatial(merged)), fun = function(x) { stats::median(x, na.rm = TRUE) })[, 2], 2)
  merged$diffHGT <- round(merged$highHGT - merged$lowHGT, 2)
  merged$maxHGT <- round(terra::extract(terra::rast(cancov), terra::vect(sf::as_Spatial(merged)), fun = max)[, 2], 2)
  ...
  # Obtain polygon attributes
  message("Obtaining polygon attributes...(10/14)\n")
  merged$TreeID <- sample(1:nrow(merged), nrow(merged), replace = FALSE)

  # Process the resulting clipped slice
  message("Processing the resulting clipped slice...(11/14)\n")
  coords <- data.frame(sf::st_coordinates(na.omit(merged)))
  circles <- lapply(unique(coords$L2), function(id) {
    circle <- data.frame(conicfit::CircleFitByLandau(coords[coords$L2 == id, c("X", "Y")]))
    names(circle) <- c("X", "Y", "R")
    circle
  })
  circles <- dplyr::bind_rows(circles)
  circles_sf <- sf::st_sf(sf::st_buffer(sf::st_cast(sf::st_sfc(sf::st_multipoint(as.matrix(circles)[, 1:2])), to = "POINT"), circles$R))
  circles_sf$R <- circles$R
  circles_sf$TreeID <- sample(1:nrow(circles_sf), nrow(circles_sf), replace = FALSE)
  circles_sf <- sf::st_buffer(circles_sf, 0.075)
  sf::st_crs(circles_sf) <- sf::st_crs(slice_extra)
  slice_clip <- lidR::merge_spatial(las = lidR::clip_roi(slice_extra, sf::st_sf(sf::st_union(circles_sf))), source = circles_sf, attribute = "TreeID")
  ...
  cyl_fit <- lapply(1:length(sort(unique(slice_clip$TreeID))), function(t) {
    min <- grid_slice_min
    max <- grid_slice_max
    if (cylinder_fit_type == "ransac") {
      ...
      n_pts <- min(data.frame(lidR::filter_poi(slice_clip, Z <= max, Z >= min)@data %>% dplyr::group_by(TreeID) %>% dplyr::summarize(count = length(X)))$count) / 2
      n_best <- 100
    } else {
      stop("You will need to choose either 'ransac' or 'irls' cylinder fitting method...", call. = FALSE)
    }
    fit <- spanner:::cylinderFit(lidR::filter_poi(slice_clip, TreeID == sort(unique(slice_clip$TreeID))[t]), method = cylinder_fit_type, n = n_pts, inliers = inliers, conf = conf, max_angle = max_angle, n_best = n_best)
