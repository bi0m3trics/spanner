# ============================================================================
# spanner – Stem_Segmentation.R
#
# Fit circles/cylinders up individual tree stems and compute stem volumes.
#
# Public functions:
#   segment_stem()        – per-tree, per-slice circle fitting → segment table (v1, deprecated)
#   compute_stem_volume() – rollup segment table to per-tree volumes
# ============================================================================


# ----------------------------------------------------------------------------
#' Segment tree stems by fitting circles across height slices
#'
#' @description
#' **Deprecated in spanner v2.0 — use [stem_segments()] instead.**
#'
#' `segment_stem` divides each segmented tree's stem into overlapping height
#' slices, fits a 2-D RANSAC circle (or Hough-seeded RANSAC circle) to the
#' stem points in each slice, and returns a tidy `data.frame` with one row per
#' (TreeID × slice). The result feeds directly into [compute_stem_volume()]
...
#'     (`C_ransac_stem_slices`) for all trees simultaneously.
...
#' }
#'
#' If the `las` object has a `Stem` column and `use_stem_column = TRUE`,
#' only stem-labelled points are used for
...
#'   output). Optionally has a `Stem` column from [stem_points()],
#'   [classify_lw_points()], or [classify_stem_points()].
...
#'   Default `0.1`.
#' @param use_stem_column logical. Use the `Stem` column, if present, to
...
#'   method preserves sub-voxel geometry for stem cross-sections. Useful for
...
#'   }
#'
#' @seealso [stem_segments()], [classify_stem_points()], [compute_stem_volume()],
#'   [assess_tree_quality()]
#'
#' @references
...
#'
#' seg_tbl  <- segment_stem(las_stem, tree_locs)
#' head(seg_tbl)
#' }
#'
#' @export
segment_stem <- function(las,
                         tree_locs,
  ...
  # Use the Stem column when present.
  stem_col <- if ("Stem" %in% names(las@data)) "Stem" else NULL
  has_stem <- use_stem_column && !is.null(stem_col)

  # ---- Optional voxel thinning (preserves stem geometry via barycenter) -----
  if (!is.null(voxel_thin) && voxel_thin > 0) {
    message("  [segment_stem] Voxel-thinning to barycenter (res = ",
            voxel_thin, " m) ...")
  ...
  if (algorithm == "ransac") {
    ...
    seg_tbl$Fit_method <- if (nrow(seg_tbl) > 0L) "ransac" else character()
    ...
    seg_tbl$Fit_method <- if (nrow(seg_tbl) > 0L) "hough" else character()
    ...
    seg_tbl$Fit_method <- if (nrow(seg_tbl) > 0L) "hough_seed_ransac" else character()
  }
...
# ============================================================================
# Internal helpers for segment_stem
# ============================================================================
...
# Build a stem-point mask (logical vector, same length as las@data rows)
.stem_mask <- function(las, tx, ty, z_lo, z_hi, sr2, has_stem, stem_col = NULL) {
  X  <- las@data$X
  Y  <- las@data$Y
  Z  <- las@data$Z
...
# ---------------------------------------------------------------------------
# RANSAC path: delegates to C++ C_ransac_stem_slices for all trees at once
# ---------------------------------------------------------------------------
.segment_ransac <- function(las, tl_xy, tl_ids,
  ...
  # Subset to stem-candidate points only (respects Stem flag)
  X  <- las@data$X
  Y  <- las@data$Y
  Z  <- las@data$Z
  ...
  # Filter out slices above each tree's own z_max (vectorised)
  z_max_lookup <- setNames(
    .per_tree_zmax(tl_ids, z_max_use, z_min),
    as.character(tl_ids)
  )
  tmax_vec <- z_max_lookup[as.character(result$TreeID)]
  keep     <- is.na(tmax_vec) | result$Z_mid <= tmax_vec
...
.segment_hough <- function(las, tl_xy, tl_ids,
  ...
  for (ti in seq_along(tl_ids)) {
    ...
    # Extract all stem-candidate points for this tree
    tree_mask <- TID == tid &
      Z >= z_min & Z <= zmax &
      (X - tx)^2 + (Y - ty)^2 <= sr2
    if (has_stem && !is.null(stem_col))
    ...
    # Run Hough on the full stem column once per tree
...
# ----------------------------------------------------------------------------
#' Compute outside-bark stem volume from a segment table
#'
#' `compute_stem_volume` aggregates the per-slice circle fits returned by
#' [segment_stem()] into per-tree outside-bark stem volume estimates. Two
#' volume formulae are available: the simple cylinder formula and Smalian's
#' formula for tapering stems.
#'
#' @param seg_table `data.frame` returned by [segment_stem()].
...
#' Compute outside-bark stem volume from a segment table
#'
#' `compute_stem_volume` aggregates the per-slice cylinder or circle fits
#' returned by [stem_segments()] or [segment_stem()] into
#' per-tree outside-bark stem volume estimates.  Two volume formulae are
#' available: the simple cylinder formula and Smalian's formula for tapering
#' stems.
#'
#' The only required columns are `TreeID`, `Segment`, `Z_low`, `Z_high`,
#' `Radius`, and `Valid`.  All other columns (`RANSAC_err`, `N_inliers`, etc.)
#' are used when present but are not required.
#'
#' @param seg_table `data.frame` returned by [stem_segments()] or`r`n#'   [segment_stem()].
#' @param method character. `"cylinder"` (default) uses `π r² h` per segment.
#'   `"smalian"` applies Smalian's formula between adjacent valid segments:
#'   `V = h × π × (r₁² + r₂²) / 2`.
#'
...
#'     \item{`Basal_area_m2`}{numeric. Basal area at the first valid segment.}
#'     \item{`Height_modeled`}{numeric. Vertical height span modeled (m).}
#'     \item{`Mean_RANSAC_err`}{numeric. Mean per-segment fit residual (m),
#'       or `NA` for methods/tables that do not report this.}
#'   }
#'
#' @seealso [stem_segments()], [tree_inventory()], [assess_tree_quality()]
#'
#' @examples
#' \donttest{
#' # (continuing from stem_segments example)
#' # segs <- stem_segments(las_s, map, method = seg_ransac_cylinder())
#' # vol  <- compute_stem_volume(segs)
#' }
#'
#' @export
compute_stem_volume <- function(seg_table,
                                method = c("cylinder", "smalian")) {

  method <- match.arg(method)

  if (!is.data.frame(seg_table))
    stop("'seg_table' must be a data.frame returned by stem_segments() or segment_stem().")
  ...
  if (length(missing_cols) > 0)
    stop("'seg_table' is missing columns: ", paste(missing_cols, collapse = ", "),
         "\nMake sure it is the output of stem_segments() or segment_stem().")
  ...
  for (i in seq_along(tree_ids)) {
    ...
    mean_err <- if ("RANSAC_err" %in% names(tr) &&
                    any(!is.na(tr$RANSAC_err))) {
      mean(tr$RANSAC_err, na.rm = TRUE)
    } else {
      NA_real_
    }
