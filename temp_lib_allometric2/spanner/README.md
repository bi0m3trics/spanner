...
The package also now includes three point-cloud tree segmentation algorithms for normalized ALS data:

- `allometric_li_geodesic()`: Li-style top-down segmentation with allometric crown envelopes and a local geodesic graph.
- `allometric_random_walker()`: Seeded diffusion-based segmentation with sparse local-candidate scoring for better speed.
...
## New canopy and woody-structure helpers
...
- `classify_lw_points()`: unified leaf/wood classification (bole, branch, and other).
- `segment_bole()`: per-tree bole circle fitting and diameter/height summaries.
- `refit_trees()`: targeted bole refit workflow for trees flagged as marginal or bad quality.
